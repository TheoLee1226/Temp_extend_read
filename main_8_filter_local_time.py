...
def read_and_process_data():
    ...
    if len(serial_values) == 9:  # 確保所有17個值都存在 ( ardiuno在一開始接收的時候需要初始畫而遺漏訊號 ) 
        try:
            current_time = datetime.datetime.now()
            
            # 如果收到的時間訊號少於五個，則增加計數器但不執行後續操作
            if time_signal_count < 5:
                time_signal_count += 1
                return
            ...
            # print(f'{relative_time:.2f}, ' + ', '.join([f'{serial_values[i+1]}' for i in range(8)]))
...
def create_csv_with_timestamp():
    try:
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            writer.writerow(['Time'] + [f'Temp_{i+1}' for i in range(8)] + [f'Temp_{j+1}_smooth' for j in range(8)])
